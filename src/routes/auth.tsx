...
function AuthPage() {
  ...
  const isBootstrap = !ownerData?.exists;
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-16">
      <h1 className="font-stamp text-3xl mb-2">
        {isBootstrap ? "Reivindicar proprietário" : "Acesso restrito"}
      </h1>
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <input
          className={INPUT}
          type="password"
          placeholder={isBootstrap ? "senha (mín. 12 caracteres)" : "senha"}
          value={password}
          onChange={(e) => setPassword(e.target.value)}
          minLength={isBootstrap ? 12 : 8}
          required
        />
        ...
        >
          ...
          {isBootstrap ? "Criar proprietário" : "Entrar"}
        </button>
      </form>
